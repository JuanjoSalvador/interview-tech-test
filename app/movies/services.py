...
class MovieService(services.APIService):
    ...
    def all(page: int = 1, page_size: int = 10, search: str = None):
        ...
        if not search:
            ...
            query = (
                base_query.select(["Type", "Title", "Year", "Poster", "imdbID"])
                .order_by("Title")
        ...
        results = {
            ...
            "page": page,
            "next-page": f"?page={page + 1}&page_size={page_size}"
            if not is_last_page
            else f"?page={page}&page_size={page_size}",
    ...
    @staticmethod
    def get(movie_id):
        query = db.collection(settings.FIRESTORE_COLLECTION).document(movie_id).get()
    ...
    def delete(movie_id, token):
        ...
        document_ref = db.collection(settings.FIRESTORE_COLLECTION).document(movie_id)
